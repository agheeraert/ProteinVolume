    ...
    def mean_std_sim_plot(self):
        ...
        for method in self.methods:
            ...
            if method[-2:] == "NN":
                ...
                mean = [np.mean(i) for i in distances]
                std = [np.std(i) for i in distances]         
            pkl.dump(mean, open(jn(self.output, 'mean_'+method+'.p'), 'wb'))
            pkl.dump(std, open(jn(self.output, 'std_'+method+'.p'), 'wb'))
            self.plot_apo_holo(mean, 'mean', method)
            self.plot_apo_holo(std, 'std', method)
            self.plot_apo_holo(mean+std, 'mean+std', method)
...
if __name__ == '__main__':
    ...
    CharacteristicDistance(['/home/aghee/PDB/prot_apo_sim'+str(i)+'_s10.dcd' for i in range(1,5)]+['/home/aghee/PDB/prot_prfar_sim'+str(i)+'_s10.dcd' for i in range(1,5)], '/home/aghee/PDB/prot.prmtop', [1000], '/home/aghee/ProteinVolume/results/', methods=['delaunay2']).mean_std_sim_plot()
