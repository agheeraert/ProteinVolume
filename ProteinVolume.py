...
class ProteinVolume():
    def __init__(self, trajs, topo, n_frames, output, methods=['delaunay'], apo_holo=True):
    ...
    def volume_Laguerre(self, frame):
    ...
    def plot_apo_holo(self, volumes):
        ...
        for i in range(len(volumes_apo)):
            ...
            plt.plot(volumes_apo[i], c='b', label="apo")
            plt.plot(volumes_holo[i], c='r', label="PRFAR")
